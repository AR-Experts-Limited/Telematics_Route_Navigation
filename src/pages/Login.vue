<template>
  <div class="loader window-height window-width bg-gray column items-center no-wrap">
...
export default {
  ...
  created () {
    if (FLESPI_TOKEN) {
      this.initConnection({ token: FLESPI_TOKEN })
        .then(() => {
          this.$router.push('/')
        })
    } else if (!this.checkHasToken()) {
      const tokenHandler = (event) => {
        // ... (existing token handler logic)
      }
